...
app.client.request = function (
  ...
  method =
    ...
    ["POST", "GET", "PUT", "DELETE", "PATCH"].indexOf(method.toUpperCase()) > -1
      ? method.toUpperCase()
      : "GET";
  queryStringObject =
  ...
  if (document.querySelector("form")) {
    ...
    for (var i = 0; i < allForms.length; i++) {
      allForms[i].addEventListener("submit", function (e) {
        ...
        var btnTextContent = document.querySelector(
          "#" + formId + " button"
        ).textContent;
        var btnOnClick = document.querySelector(
          "#" + formId + " button"
        ).onclick;
        document.querySelector("#" + formId + " button").textContent =
          "Loading...";
        document.querySelector("#" + formId + " button").onclick = false;
        ...
        app.client.request(
          ...
          function (statusCode, responsePayload) {
            ...
            if (statusCode !== 200 && statusCode !== 201) {
              ...
              document.querySelector("#" + formId + " button").textContent =
                btnTextContent;
              document.querySelector("#" + formId + " button").onclick =
                btnOnClick;
...
app.formResponseProcessor = function (formId, requestPayload, responsePayload) {
  ...
  } else if (formId == "shoppingPay") {
    alert("Your shopping car was processed correctly");
...
app.getItems = function (callback) {
  app.getSessionToken();
  let sessionToken = app.config.sessionToken;
...
app.showItems = function (elementID, arr, clear) {
  const element = document.getElementById(elementID);
  if (clear) element.innerHTML = "";

  const template = ({ id, name, description, currency, price, url }) => `
  <div class="blurb">
    ...
    <div class="description-container">
      ...
      <div class="business-container">
        ...
        <div class="add-car-btn" onClick="app.addToCar('${id}')">Add to car</div>
      </div>
    </div>
  </div>
  `;
...
app.addToCar = function (id, units, reload) {
  reload = reload || false;
  units = units || 1;
  let sessionToken = app.config.sessionToken;
  if (!sessionToken) return;

  let quantity = prompt("How many units do you want?", units);
  quantity = parseInt(quantity);
  if (!quantity || quantity < 1)
    return alert("Enter a positive integer value please.");
  ...
  app.client.request(
    ...
    function (statusCode, responsePayload) {
      if (statusCode !== 200)
        return alert(
          "Something wrong, your order was nor processed correctly. Try again please."
        );
      alert("Nice! You shopping car was updated correctly");
...
app.addPayNowEvent = function () {
  ...
  paynowbtn.addEventListener(
    ...
    function (event) {
      ...
      let sessionToken = app.config.sessionToken;
      if (!sessionToken) return;
      window.location = `/shopping-car?token=${sessionToken.id}&email=${sessionToken.email}`;
    },
    false
  );
};
